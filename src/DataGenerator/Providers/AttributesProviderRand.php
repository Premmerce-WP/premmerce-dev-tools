...
class AttributesProviderRand extends Base{
	...
	public function mod(){
		return $this->generator->randomKey($this->base);
	}

	private function getMod($mod = null){
		if($mod && isset($this->base[ $mod ])){
			return $mod;
		}
	...
	protected function iterate($num = 1, $callback, $string = false){
		$items = [];
		...
		return $string? implode(',', $items) : $items;
	}

	public function val($mod = null, $num = 1, $string = false){
		$mod = $this->getMod($mod);
		$val = $this->base[ $mod ]['value'];

		return $this->iterate($num, function() use ($val){
			return $this->generator->parse($val);
		}, $string);
	}

	public function nm($mod = null){
		$mod = $this->getMod($mod);

		$additional = $this->base[ $mod ];

		$name = $this->generator->randomElement($additional['base']);

		if($this->generator->boolean){
			$name = $this->generator->randomElement($additional['prefix']) . ' ' . $name;
		}
		if($this->generator->boolean){
			$name .= ' ' . $this->generator->randomElement($additional['suffix']);
		}

		return $name;
	}

	public function stripAttribute($name){

		if($mod = $this->getAttributeModifier($name)){
	...
	public function getAttributeModifier($name){
		$lpos = strpos('(', $name);
		$rpos = strpos(')', $name);
